...
for i in range(5):
    easter_holliday += [easter(this_year + i)]

for date in easter_holliday:
        print(f"Easter sunday in {date.year} is on {date.strftime("%d.%m.%Y")}")
...
def next_christmas():
    this_year = datetime.now().year
    while True:
        xmas_date = date(this_year, 12, 25)
        if xmas_date.weekday() == 6:
            return this_year
        this_year += 1

next_xmas = next_christmas()

print(f"The next Christmas on sunday is in year {next_xmas}\n.")